...
def nmap_scan(ip):
    if "/" in str(ip): 
        ...
        targets = ip.split('/')[0]
        result = subprocess.run(['nmap','-sn','-oA',f'nmap/{targets}.hosts.up',ip], capture_output=True, text=True)
        for line in result.stdout.splitlines():
            if 'Nmap scan report for' in line:
                live_hosts.append(line.split()[-1])

    else:
        subprocess.run(['nmap','-Pn','--min-rate=500','--max-retries=3','--max-rtt-timeout=900ms', '-T4', '-sS', '-sV','-dd','-oA',f'nmap/{ip}.tcp',ip])
